...
from .utils import trans_from_transform, rot_from_transform, transform_from_rot_trans, compute_depth_range, to_numpy
    ...
    def __call__(self, sample):
        ...
        if "images" in sample:
            ...
            if isinstance(images, list):
                ...
                images = resize(images, list(images.shape[:-2]) + [ht, wd], order=self.__interpolation_order)
            else:
                raise ValueError("Invalid type for images.")
            sample["images"] = images
        
        # resize intrinsics:
        if "intrinsics" in sample:
            scale_arr = np.array([[wd / orig_wd]*3, [ht / orig_ht]*3, [1.]*3], dtype=np.float32)  # 3, 3
            if isinstance(sample["intrinsics"], list):
                sample["intrinsics"] = [intrinsic * scale_arr for intrinsic in sample["intrinsics"]]
            elif isinstance(sample["intrinsics"], np.ndarray):
                sample["intrinsics"] = sample["intrinsics"] * scale_arr
            else:
                raise ValueError("Invalid type for intrinsics.")
...
class ResizeTargets:
    ...
    def __call__(self, sample):
        ...
        attributes = [ 'cam_normal', 'world_normal', 'cam_coord', 'world_coord', 'mask']

        for attr in attributes:
            # assert attr in sample, f"{attr} not in sample"
            if not attr in sample:
                continue
            attr_val = sample[attr]
            if isinstance(attr_val, list):
                attr_val = [resize(x, list(x.shape[:-2]) + [ht, wd], order=self.__interpolation_order, anti_aliasing=False, preserve_range=True) for x in attr_val]
            elif isinstance(attr_val, np.ndarray):
                attr_val = resize(attr_val, list(attr_val.shape[:-2]) + [ht, wd], order=self.__interpolation_order, anti_aliasing=False, preserve_range=True)
            else:
                raise ValueError(f"Invalid type for {attr}.")
            sample[attr] = attr_val
        return sample
